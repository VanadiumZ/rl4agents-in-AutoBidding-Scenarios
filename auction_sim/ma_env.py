...
class MultiAgentAuctionEnv:
    ...
    def _setup_spaces(self):
        ...
        # Action space: continuous bid multiplier [0.0, 1.5] (允许弃标和保守出价)
    ...
    def reset(self) -> Dict[str, np.ndarray]:
        ...
        # Generate and store initial observations for time consistency
        self.last_observations = self._get_observations()
    ...
    def step(self, actions: Dict[str, np.ndarray]) -> Tuple[Dict, Dict, Dict, Dict, Dict]:
        ...
        # Use last observations for current action evaluation (fix time consistency)
        current_observations = self.last_observations
        
        # Collect all bids using current observations
        all_bids = {}
        perceived_values = {}
        ...
        rewards = {}
        complex_rewards = {}  # 存储复合奖励用于分析
        for agent_id in self.learning_agent_ids:
            reward, profit = self._calculate_reward(agent_id, auction_results, perceived_values.get(agent_id, 0))
            complex_reward, _ = self._final_objective_reward(agent_id, auction_results, perceived_values.get(agent_id, 0))
            rewards[agent_id] = reward
            complex_rewards[agent_id] = complex_reward
        ...
        # Generate next observations for next step
        ...
        info = {agent_id: {
            ...
            'round': self.current_round,
            'complex_reward': complex_rewards.get(agent_id, 0.0)  # 复合奖励用于分析
        } for agent_id in self.learning_agent_ids}
