...
class LearningAgent(Agent):
    """"自适应"学习智能体：使用强化学习模型进行出价 (k=1场景)"""
    def __init__(self, agent_id: str, budget: float, perception_noise_std: float):
        super().__init__(agent_id, budget, perception_noise_std)
        # TODO: 在这里初始化你的RL模型 (e.g., PPO from stable-baselines3)
        # self.model = PPO(...)
        # self.replay_buffer = ReplayBuffer(...)
        print(f"LearningAgent {self.id} initialized (k=1 single-agent scenario).")
...
class MultiAgentLearningAgent(Agent):
    ...
    Designed to work with MAPPO/IPPO algorithms
    """
    def __init__(self, agent_id: str, budget: float, perception_noise_std: float, 
                 model=None, is_training: bool = True):
        super().__init__(agent_id, budget, perception_noise_std)
        self.model = model  # Will be set externally by training framework
        self.is_training = is_training
        self.last_observation = None
        self.last_action = None
        
        # Multi-agent specific tracking
        self.win_history = deque(maxlen=100)  # Track recent wins for observation
    ...
    def get_observation(self, perceived_value: float, current_round: int, max_rounds: int, 
        ...
        This mirrors the observation building in ma_environment.py
        """
        from . import config
        ...
        competition_level = 0.6  # Assume moderate competition
    ...
    def bid(self, perceived_value: float, current_round: int = 1, max_rounds: int = 1000, 
        ...
        try:
            ...
            if hasattr(self.model, 'predict'):
                # For stable-baselines3 style models
            ...
            # Ensure action is in valid range (matching network output)
    ...
    def update(self, result: Dict, round_num: int, true_value: float = None, profit: float = None):
        ...
        if self.is_training and hasattr(self, '_store_experience'):
            self._store_experience(result, profit)
    
    def _store_experience(self, result: Dict, profit: float):
        """
