# /auction_sim/ippo_runner.py
"""
IPPO Training Runner for k=2 scenario
Trains 2 learning agents using Independent PPO (no coordination)
"""
import numpy as np
import random
import torch
...
def create_learning_agents():
    """Create learning agents for IPPO training"""
    learning_agents = []
    learning_agent_ids = []
...
def train_ippo(n_episodes: int = 100, save_models: bool = True):
    """
    Main training function for IPPO multi-agent scenario
    """
    print("="*60)
    print("INDEPENDENT PPO TRAINING (k=2)")
    print("="*60)
    ...
    print(f"Created {len(learning_agents)} independent learning agents")
    ...
    # Training loop
    print(f"Starting IPPO training for {n_episodes} episodes...")
    
    best_avg_reward = float('-inf')
    episode_rewards_history = []
    
    for episode in tqdm(range(n_episodes), desc="IPPO Training Episodes"):
        ...
        if avg_reward > best_avg_reward:
            ...
            if save_models:
                trainer.save_models("auction_sim/models/ippo_best")
        ...
        if episode % 10 == 0:
            ...
            print(f"\nIPPO Episode {episode} (full {max_steps} rounds):")
    ...
    # Save final models
    if save_models:
        trainer.save_models("auction_sim/models/ippo_final")
        trainer.plot_training_curves("auction_sim/results/ippo_training_curves.png")
    
    print("\n" + "="*60)
    print("IPPO TRAINING COMPLETED")
    print("="*60)
    
    return trainer, learning_agents, rule_agents

def evaluate_ippo_agents(trainer: IPPOTrainer, n_eval_episodes: int = 3):
    """
    Evaluate trained IPPO agents against rule-based opponents
    """
    print("\n" + "="*50)
    ...
    for i, agent in enumerate(learning_agents):
        ...
        eval_wrapper = IPPOEvalWrapper(trainer.networks[agent_id], trainer, agent_id)
    ...
    for episode in range(n_eval_episodes):
        ...
        print(f"IPPO Eval Episode {episode + 1}: Rewards = {episode_rewards}, Wins = {episode_wins}")
    ...
    print(f"\nIPPO Evaluation Results (averaged over {n_eval_episodes} episodes):")
    print(f"Average Rewards: {avg_rewards}")
    print(f"Average Win Rates: {avg_win_rates}")
    
    return avg_rewards, avg_win_rates

def run_ippo_experiment():
    """
    Run complete IPPO experiment: train, evaluate, and visualize
    """
    ...
    # Training phase
    trainer, learning_agents, rule_agents = train_ippo(n_episodes=100)
    
    # Evaluation phase
    avg_rewards, avg_win_rates = evaluate_ippo_agents(trainer, n_eval_episodes=3)
    
    # Create agents for final simulation with trained models
    print("\n" + "="*50)
    print("RUNNING FINAL IPPO SIMULATION")
    print("="*50)
    ...
    print("Running final IPPO simulation to generate agent histories...")
    for step in tqdm(range(config.SIMULATION_ROUNDS), desc="Final IPPO Simulation"):
        actions = {}
    ...
    print("\n✅ IPPO experiment completed successfully!")
